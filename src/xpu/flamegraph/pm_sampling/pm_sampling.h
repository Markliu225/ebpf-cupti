...
public:
    ...
    CUptiResult EvaluateCounterData(CUpti_PmSampling_Object* pSamplingObject, size_t rangeIndex, std::vector<const char*> metricsList, std::vector<uint8_t>& counterDataImage)
    {
        ...
        samplerRange.rangeIndex = rangeIndex;
    ...
    void PrintSampleRanges()
    {
        ...
        size_t limit = std::min<size_t>(50, m_samplerRanges.size());
        for (size_t sampleIndex = 0; sampleIndex < limit; ++sampleIndex)
        {
    ...
    void PrintLastSamples(size_t lastN)
    {
        if (lastN == 0 || m_samplerRanges.empty())
        {
            return;
        }
        size_t total = m_samplerRanges.size();
        size_t start = (lastN >= total) ? 0 : (total - lastN);
        for (size_t sampleIndex = start; sampleIndex < total; ++sampleIndex)
        {
            const auto& samplerRange = m_samplerRanges[sampleIndex];
            std::cout << "Sample Index: " << sampleIndex << "\n";
            std::cout << "Timestamps -> Start: [" << samplerRange.startTimestamp << "] \tEnd: [" << samplerRange.endTimestamp << "]" << "\n";
            ...
            // Emit an additional machine-readable line for tooling to parse quickly.
            uint64_t duration = 0;
            if (samplerRange.endTimestamp >= samplerRange.startTimestamp)
            {
                duration = samplerRange.endTimestamp - samplerRange.startTimestamp;
            }

            std::ostringstream sampleLine;
            sampleLine << "SAMPLE_ROW";
            sampleLine << " range_index=" << samplerRange.rangeIndex;
            sampleLine << " start_timestamp=" << samplerRange.startTimestamp;
            sampleLine << " end_timestamp=" << samplerRange.endTimestamp;
            sampleLine << " duration_ns=" << duration;
            for (const auto& metric : samplerRange.metricValues)
            {
                sampleLine << ' ' << metric.first << '=' << metric.second;
            }
            std::cout << sampleLine.str() << std::endl;
        }
    }

    bool ExportSamplesToCsv(const std::string& filePath, const std::vector<const char*>& metricsList) const
    {
        std::ofstream csv(filePath, std::ios::trunc);
        if (!csv.is_open())
        {
        ...
        metricHeaders.reserve(metricsList.size());
        for (const char* metricName : metricsList)
        {
            metricHeaders.emplace_back(metricName ? metricName : "");
        }

        csv << "range_index,start_timestamp,end_timestamp,duration_ns";
        for (const auto& header : metricHeaders)
        {
            csv << ',' << header;
        }
        csv << '\n';
        ...
        for (const auto& samplerRange : m_samplerRanges)
        {
            ...
            csv << samplerRange.rangeIndex << ',';
            csv << samplerRange.startTimestamp << ',';
            csv << samplerRange.endTimestamp << ',';
            csv << duration;

            for (const auto& header : metricHeaders)
            {
                csv << ',';
                auto it = samplerRange.metricValues.find(header);
                if (it != samplerRange.metricValues.end())
                {
                    csv << it->second;
                }
            }
