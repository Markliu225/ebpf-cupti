...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="GPU MLP Task A")
    # [新增] 角色控制
    ...
    parser.add_argument("--feature-dim", type=int, default=64, help="输入特征维度")
...
def train_epoch(model, optimizer, args, device, epoch_idx):
    ...
    # 仅在 Target 模式或 Noise 的部分时候打印，避免日志爆炸
    if args.role == 'target' or (epoch_idx % 10 == 0):
        print(f"[Task A] Epoch {epoch_idx} | loss={avg_loss:.4f} | time={elapsed:.3f}s")
...
def main() -> int:
    ...
    print(f"[Task A] Role: {args.role} | Device: {device}")

    # === [核心逻辑修改] ===
    if args.role == 'noise':
        # 模式：噪声 (死循环)
        print("[Task A] Running infinite training loop as NOISE...")
        epoch_counter = 1
        try:
            while True:
        ...
        # 模式：目标 (测量)
        print(f"[Task A] Running {args.epochs} epochs as TARGET...")
