...
def train(args, device: torch.device, logger) -> None:
    ...
    model = models.resnet50(weights=None, num_classes=int(args.num_classes)).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for _ in range(steps):
            ...
            y = torch.randint(0, int(args.num_classes), (bsz,), device=device, dtype=torch.long)
        ...
        samples = steps * bsz
        logger.info(
            "train epoch=%d loss=%.6f steps=%d samples=%d samples_per_s=%.2f amp=%s channels_last=%s",
            epoch + 1,
    ...
    ckpt_path = Path(args.artifact_dir) / "synthetic_resnet50.pt"
    ckpt_path.parent.mkdir(parents=True, exist_ok=True)
    torch.save({"model_state": model.state_dict(), "num_classes": int(args.num_classes)}, ckpt_path)
    logger.info("checkpoint saved path=%s", ckpt_path)


def infer(args, device: torch.device, logger) -> None:
    ckpt_path = args.checkpoint or Path(args.artifact_dir) / "synthetic_resnet50.pt"
    ...
    dt = time.perf_counter() - t0
    samples = steps * bsz
    logger.info("infer steps=%d samples=%d samples_per_s=%.2f mean=%.6f amp=%s", steps, samples, samples / max(1e-9, dt), acc / max(1, steps), use_autocast)
